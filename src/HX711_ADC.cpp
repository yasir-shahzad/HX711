...
HX711_ADC::HX711_ADC(uint8_t dout, uint8_t sck) //constructor
          : doutPin(std::to_string(dout)), sckPin(std::to_string(sck))
{
	sckPin.Enable();
	doutPin.Enable();
...
{
	sckPin.Mode(OUTPUT); // Set signalPin as output
	doutPin.Mode(INPUT); // Set data as input
	setGain(128);
	powerUp();
}
...
{
	uint8_t dout = doutPin.Read(); //check if conversion is ready
	if (!dout) 
	{
		conversion24bit();
		lastDoutLowTime = millis();
...
void HX711_ADC::conversion24bit()  //read 24 bit data, store in dataset and start the next conversion
{
	...
	{ 	//read 24 bit data + set gain and start next conversion
		sckPin.Write(HIGH);
		// if(SCK_DELAY) usleep(1); // could be required for faster mcu's, set value in config.h
		sckPin.Write(LOW);
		if (i < 24) 
		{
			dout = doutPin.Read();
...
bool HX711_ADC::refreshDataSet()
{
	...
	while ( s > 0 ) {
		update();
		if (doutPin.Read() == 0) { // HX711 dout pin is pulled low when a new conversion is ready
...
unsigned long HX711_ADC::millis() {
	unsigned long value_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - refTime).count();
	return value_ms;
}

unsigned long HX711_ADC::micros() {
	unsigned long value_us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - refTime).count();
	return value_us;
}
